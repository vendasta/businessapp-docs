...
5. Click the pencil icon at the right side of the row with the `Name` "@", make sure it is an `A` record, and change the value to `34.149.86.124`.
...
8. To encourage the DNS system on the internet to update quickly you can flush Google's DNS cache using the [Google Public DNS Flush Cache](https://developers.google.com/speed/public-dns/cache) tool.
