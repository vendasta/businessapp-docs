...
description: View and manage all your social and blog posts directly in Business App without switching to the Social Marketing product.
tags: [social, business-app, manage-posts]
keywords: [business app, manage social posts, drafts, scheduled posts, published posts]
---
...
- Schedule draft posts directly from draft cards
...
- Direct scheduling from **Draft** post cards
...
- In `Drafts`, click `Schedule` on a draft post card to schedule that post

![Manage Posts Business App Social Published View](img/manage-posts/Manage-Tabs-Socia-Business-App-Published-View.png)


### Step 4: Schedule a draft post

You can schedule draft social posts directly from the `Drafts` tab in both desktop and mobile views.
This is available for single-location and multi-location Social accounts.

1. Open `Social` in Business App and select the `Drafts` tab.
2. Choose a post card with a valid scheduled date and time.
3. Click `Schedule`.
4. A confirmation message appears after the post is scheduled.

:::warning
- The `Schedule` button appears only for posts in the `Drafts` state.
- If the scheduled date and time are earlier than the current time, `Schedule` is disabled and shows: `Please select a future date and time to schedule this post`.
- If the scheduled time is earlier than the current time and no valid social network is connected, the tooltip shows: `Please ensure the scheduled time is in the future and at least one social account is connected`.
:::
...
:::warning
- You **cannot edit post content** for draft or scheduled posts within the Business App.  
  To make edits, open the post in the **Social Marketing** product.  
- You can schedule posts directly from draft post cards in Business App.
...
You can schedule draft posts and delete posts in Business App. To edit post content, open the post in the Social Marketing product.
</details>

<details>
<summary>Why is the Schedule button disabled on a draft post?</summary>

The `Schedule` button is disabled when the scheduled date and time are earlier than the current time. It can also remain disabled if no valid social account is connected.
</details>
