...
- Image generation is managed dynamically based on API capacity.
- Daily image generation capacity scales based on available API limits.
- These limits apply to image generation workflows in Social Marketing on both **web** and **mobile**.
...
Social Marketing uses GPT Image 1 to generate images.
</details>