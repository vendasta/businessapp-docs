---
title: Managing Social Connections in Business App Integrations
sidebar_label: Social connections
description: Connect and manage all your social media accounts directly from Business App Integrations, without navigating to Social AI.
sidebar_position: 6
tags: [social-ai, settings, integrations, connections]
keywords: [social connections, business app integrations, connect facebook, connect instagram, connect linkedin, connect tiktok, connect youtube, wordpress blog, custom feed, disconnect account]
---

## What are Social Connections in Business App Integrations?

You can connect and manage all your social media accounts directly within the Business App, without navigating to the Social AI product. Everything is managed from `Administration` > `App Settings` > `Integrations`, giving you a single place to set up and maintain your connections.

## Why are Social Connections important?

- **One place for everything**: Set up and maintain all your social connections from Business App Integrations.
- **Connect more platforms**: Add a wide range of social networks and content sources.
- **Faster setup**: Establish connections without switching to another product.
- **Easy to maintain**: Review connected accounts and disconnect any you no longer need.

## Which platforms can you connect?

You can connect the following from Business App Integrations:

- **Facebook**
- **Google Business Profile**
- **LinkedIn**
- **X**
- **Instagram**
- **TikTok**
- **YouTube**
- **WordPress Blog**
- **Custom Feed**

## How to connect a social account

### Step 1: Open the Business App

Sign in to your Business App.

![Business App home page](img/business-app-integrations/Business-App-Home.png)

### Step 2: Go to Administration

Navigate to `Administration`.

![Business App Administration](img/business-app-integrations/Business-App-Administration.png)

### Step 3: Open Integrations

Click `App Settings` > `Integrations`.

### Step 4: Select the Browse tab

Select the **Browse** tab to view the available platforms.

![Browse tab in Business App Integrations](img/business-app-integrations/Business-App-Integrations-Browse.png)

### Step 5: Choose a platform

Choose the platform you want to connect, such as Instagram, LinkedIn, X, TikTok, YouTube, WordPress Blog, or Custom Feed.

### Step 6: Review the Manage Network screen

The **Manage Network** screen opens, displaying the **Add Connection** button along with your connected accounts, a description, details, and associated products.

![Manage Network screen in Business App Integrations](img/business-app-integrations/Business-App-Integrations-Manage-Connection.png)

### Step 7: Add the connection

Click **Add Connection**.

![Add Connection in Business App Integrations](img/business-app-integrations/Business-App-Integrations-Add-Connection.png)

### Step 8: Sign in to the platform

You are redirected to the platform's sign-in and authorization screen.

### Step 9: Authorize access

Enter your credentials and authorize access. Your connection is now established.

## Removing a connection

1. Go to `Settings` > `Integrations` > `Manage`.

![Business App Integrations Manage](img/business-app-integrations/Business-App-Integrations-Manage.png)

2. Select the connection you want to remove.

![Business App Integrations Remove Connection](img/business-app-integrations/Business-App-Integrations-Remove-Connection.png)

3. Click **Disconnect**. The connection is removed.

For more details on connecting and managing accounts inside the product, see [Connecting and managing accounts in Social AI](./account-connections.mdx).

## Frequently Asked Questions (FAQs)

<details>
<summary>Where do I manage my social connections?</summary>

Go to `Administration` > `App Settings` > `Integrations`, then open the **Browse** tab.
</details>

<details>
<summary>Which platforms can I connect from Business App Integrations?</summary>

Facebook, Google Business Profile, LinkedIn, X, Instagram, TikTok, YouTube, WordPress Blog, and Custom Feed.
</details>

<details>
<summary>Do I need to go to Social AI to connect an account?</summary>

No. You can connect and manage your social accounts directly from Business App Integrations.
</details>

<details>
<summary>How do I add a connection?</summary>

On the **Browse** tab, choose a platform, click **Add Connection**, then sign in and authorize access on the platform's screen.
</details>

<details>
<summary>What does the Manage Network screen show?</summary>

It displays the **Add Connection** button along with your connected accounts, a description, details, and associated products.
</details>

<details>
<summary>What happens when I click Add Connection?</summary>

You are redirected to the platform's sign-in and authorization screen to enter your credentials and grant access.
</details>

<details>
<summary>How do I remove a connection?</summary>

Go to `Settings` > `Integrations` > `Manage`, select the connection, and click **Disconnect**.
</details>

<details>
<summary>Where can I see which accounts are connected?</summary>

Open the **Manage Network** screen for a platform, or the **Manage** tab, to review your connected accounts.
</details>

<details>
<summary>Can I connect a WordPress blog or a custom feed here?</summary>

Yes. Both **WordPress Blog** and **Custom Feed** are available to connect from Business App Integrations.
</details>
