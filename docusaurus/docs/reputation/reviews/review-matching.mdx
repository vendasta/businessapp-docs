...
tags: [reputation-ai, review-matching, customer-feedback]
...
In a matched contact’s timeline, the review activity includes:
...
In `Conversations`, Reputation events are also shown as inbound timeline activities:

- **Review Matched**: Displays as an inbound event with the title `Review Matched`.
- **NPS Feedback**: Displays as an inbound event with the title `Contact has submitted NPS survey.`
- **NPS feedback formatting**: Multiline feedback keeps the original line breaks in the timeline.
- **Event icons**: Each event type includes its own icon so you can quickly identify the activity type.
Review-matched activity also appears in the `Conversations AI` timeline as system activity for the matched contact.
...
Contact support to detach the review from the incorrect contact.
</details>
