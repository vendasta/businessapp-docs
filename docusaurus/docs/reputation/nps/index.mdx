...
Sending direct review requests to all customers may result in negative public feedback, especially from dissatisfied clients. NPS surveys offer a buffer by letting you gather private feedback first. By identifying and responding to detractors privately, you reduce the risk of negative public reviews and strengthen your overall online reputation.
Sending direct review requests to all customers may result in negative public feedback, especially from dissatisfied clients. NPS surveys offer a way to gather private feedback first. By identifying and responding to detractors privately, you reduce the risk of negative public reviews and strengthen your overall online reputation.
...
You can use the default SMS request flow for NPS outreach.
:::
...
You can export NPS responses from the `ML Data Exporter` tab.

1. Go to `Reputation AI` > `Analytics` > `ML Data Exporter`.
2. In the data type dropdown, select `NPS`.
3. Review the NPS table. The table columns update for NPS data.
4. Apply filters to narrow results. You can use the same filter behavior available for reviews, including the `Page` filter in single-location view.
5. Export the filtered data.

NPS export is visible only when this option is enabled for your account.
NPS feedback entries also appear in the `Conversations AI` timeline as system activity for the related contact. This gives you one place to track scores, comments, and other conversation activity.
...
No. The SMS message is hardcoded and is not customizable.
</details>

<details>
<summary>Why don't I see NPS in the ML data exporter dropdown?</summary>

The NPS export option appears only when it is enabled for your account.
</details>
...
<details>
<summary>Can I export NPS data?</summary>

Yes. Go to `Reputation AI` > `Analytics` > `ML Data Exporter`, select `NPS`, apply filters, and export your data.
</details>
