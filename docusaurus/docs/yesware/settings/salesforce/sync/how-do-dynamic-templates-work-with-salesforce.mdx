...
sidebar_label: "Dynamic Templates"
description: "How Do Dynamic Templates Work with Salesforce?"
sidebar_position: 4
---

Yesware’s dynamic templates allow you to personalize emails using live data pulled from Salesforce Contact or Lead records, if you're on the Enterprise plan. When a match is found, fields in your template are automatically filled in with real-time information, making it easy to send personalized communication at scale.
...
If a matching Contact or Lead is found in Salesforce, the dynamic fields automatically populate with that record's data.
...
- **Outlook**: Click on the template name to trigger the Salesforce lookup. Once fields are filled, click `Insert`.

Using the `Insert` button too early (without previewing or selecting the template) inserts the template as-is, with merge fields left blank.

## Inserting Salesforce fields into a template

When building or editing a template, you can insert Salesforce merge fields:

1. Click the `Insert Merge Field` button at the bottom of the template editor.
2. Select `Salesforce Field`.
3. Begin typing the API name of the field you'd like to use. Matching options are suggested.
