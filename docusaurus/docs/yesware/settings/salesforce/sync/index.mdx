...
On the Enterprise plan, you can personalise templates using live data from Salesforce Contact or Lead records. When you select a template, Yesware looks up the first recipient in the To field and auto-fills any dynamic fields with matching Salesforce data.
...
Yesware picks one record to sync to. It prefers Leads over Contacts, then Contacts with open Opportunities, then the record with the most recent activity, then the most recently modified record. If you use Gmail and are on the Enterprise plan, you can use the **Sync Selector** to manually choose which record to associate.
