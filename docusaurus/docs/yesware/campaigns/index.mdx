...
Campaigns enables you to create multi-touch, multi-channel outreach sequences directly from your inbox. It's similar to a mail merge, but with additional options like phone calls, LinkedIn tasks, and automated or manual email touches.
...
**Gmail:** Campaigns are built and managed directly from the Gmail interface via the add-on.

**Outlook:** Campaigns are accessed via the Add-ins section in your Outlook ribbon, and open in a standalone window for full functionality.
...
### Understanding email delivery in campaigns

When you schedule automated emails in a campaign, it's important to understand how delivery works:

**Yesware hands off emails to your email provider:** Yesware doesn't send emails directly. Instead, when you schedule a campaign, Yesware queues the emails and hands them off to your email provider (Gmail or Outlook) for actual delivery.

**Expect time windows, not exact times:** Emails are delivered during a time window rather than at a precise moment. The actual send time can vary based on:

- Your email provider's server load and availability
- The size of your campaign (larger campaigns may take longer to process)
- Email sending limits and throttling applied by your provider

This handoff approach helps ensure better deliverability and prevents your emails from being flagged as spam.
...
<details>
<summary>Is there a way to stagger emails within a campaign?</summary>

Campaign emails are queued and handed off to your email provider (Gmail or Outlook) for delivery. The emails are typically processed about 10-15 seconds apart when sending to a list of recipients.

**Important to note:** Yesware doesn't directly send the emails. Instead, it hands them off to your email provider for delivery during the scheduled time window. Because of this:

- Actual delivery times may vary based on email server availability and load
- Larger campaigns may take longer to process completely
- Delays can occur during high-traffic periods

While it's a little more time-consuming, breaking up a large list into a few smaller ones and setting different send time windows for each batch when you add them to Campaigns is typically recommended. This helps manage expectations and provides better control over the delivery flow.

</details>

<details>
<summary>How are monthly campaign recipient limits calculated?</summary>
...
<details>
<summary>Why can't I import a CSV file?</summary>

This usually happens when the CSV is saved as **CSV UTF-8 (Comma delimited)** instead of the standard **CSV (Comma delimited)** format. Although both end with `.csv`, they are different formats.
