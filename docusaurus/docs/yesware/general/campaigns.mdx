...
**NOTE:** If the REMOVE button is greyed out, check the 'Status' of the recipient. The remove button works by removing ACTIVE recipients from the campaign's sequences. The RECIPIENTS tab is a log of every recipient that was added to the campaign at some point.
