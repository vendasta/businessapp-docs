...
3. Set the capability name to `Support HEARD Framework`
...
Adjust the role prompt, HEARD capability, or `Capture leads` capability based on what you observe. Common refinements include adjusting the bridge language to match your brand voice or adding specific topics to the `Capture leads` prompt that should always trigger a message.
