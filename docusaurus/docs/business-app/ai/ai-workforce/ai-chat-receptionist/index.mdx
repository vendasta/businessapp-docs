...
description: "Set up the Business App AI Chat Receptionist to answer questions and capture leads 24/7 on web chat, email, SMS, Facebook, Instagram, and WhatsApp."
sidebar_label: AI Chat Receptionist
sidebar_position: 2
keywords: [AI chat receptionist, website chatbot, email bot, SMS chatbot, Facebook Messenger bot, WhatsApp business bot, live chat AI, lead capture bot, 24/7 chat support, AI customer service, multi-channel chat, conversational AI, automated messaging, auto-reply email]
...
kb_id: KB-00051
answer_snippet: "The AI Chat Receptionist in Business App answers questions and captures leads from your website visitors 24/7, and can also reply on email, SMS, Facebook, Instagram, and WhatsApp. To set it up, go to AI Workforce in the AI menu and click Configure on the Chat receptionist."
...
The AI Chat Receptionist in Business App answers questions and captures leads from your website visitors 24/7, and can also reply on email, SMS, Facebook, Instagram, and WhatsApp. To set it up, go to `AI Workforce` in the `AI` menu and click `Configure` on the Chat receptionist.
...
>
  ...
  >
    <iframe
      ...
      title="AI Chat Receptionist overview video"
...
It also handles multiple visitors at once, delivering timely responses that guide them through your site's information.
 
## How do I set up the AI Chat Receptionist? {#set-up-the-ai-chat-receptionist}

Follow these steps to configure your AI Chat Receptionist so it engages visitors and captures leads.
...
- **Role**
The Role prompt sets the tone and behavior of your AI Chat Receptionist. Include instructions about your business's voice, how the AI should greet visitors, and any important policies or information it should share. This is where you define what makes your AI Chat Receptionist uniquely suited to your business.

- **Triggers**
Your AI Chat Receptionist is assigned to the **Web Chat** trigger by default. This trigger is always active and cannot be disabled. You can also assign your AI Receptionist to additional triggers like **Email**, **SMS**, **Facebook**, **Instagram**, and **WhatsApp** to automatically respond to inbound customer messages across all your communication channels.
...
To enable email auto-response, simply enable the Email trigger in your AI Chat Receptionist settings. Learn more about [setting up Conversations for Emails](/business-app/conversations/email/).
:::

:::tip Respond differently on each channel
Your AI Chat Receptionist knows which channel it's replying on, so it can adapt its tone and reply length per channel. To set this up, add channel-specific instructions to your **Role** prompt (below) or to a capability's instructions — for example, "When responding on SMS, keep replies to one or two sentences with no formatting; when responding by email, write longer, more detailed replies." Both places work. See [Adjust responses by channel](../ai_workforce_overview.md#adjust-responses-by-channel) for examples.
:::

#### Example Role prompts by business type {#example-purpose-by-business-type}

Use the examples below as starting points for your **Role** prompt. Pick the tab that best matches your business type (or choose **Generic**), copy the prompt into your Role field, and tailor the voice, services, and policies to your brand. Keep it specific and concise. The AI will follow these directions in every conversation.
...
- **Collect Marketing Consent**  
  The AI can ask visitors for marketing consent during a conversation. Consent responses are captured during lead capture and written directly to the contact record in your CRM. To enable this, add consent-collection instructions to your AI's Role or a custom capability.

- **Book Appointments**  
  Connect your calendar to let the AI help customers schedule meetings. It will offer available time slots and collect all necessary booking details automatically. Customers can book multiple services in a single conversation, completing the process with SMS confirmation.
...
You can let your AI Chat Receptionist book multiple services in one conversation when those services are part of the same `Service Menu/Group`.

To enable multi-service booking:

1. Go to `AI` > `AI Workforce` > `Chat Receptionist` > `Configure`.
2. In `Capabilities`, add `Book Multiple Services`.
3. Select the `Service Menu/Group` the AI should use.
4. Keep `Book Appointments` enabled and select the booking link the AI should use.
5. Click `Save`, then test in Web Chat with a request that includes more than one service.

When enabled:
- The AI confirms the services before booking.
- The AI looks for back-to-back availability for the full session.
- If no combined slot is available, the AI offers alternatives, such as a different time or splitting the booking.
- The AI collects all required contact and intake details before confirmation.
...
#### Example additional instructions capabilities by business type {#example-custom-capabilities-and-additional-instructions-by-business-type}
 ...
 The examples below show how you can use simple text instructions in Capabilities to help your AI Chat Receptionist understand how to handle different customer types. These complement your Role prompt by focusing on operational requirements, qualification data, routing rules, and response formats.
...
**Example C — Travel planning service:**
...
:::info AI Receptionist Taking Actions 
 ...
 Getting your AI Receptionist to take actions requires adding the appropriate Tool to your Custom Capabilities. For more information on Tools, see [Creating Custom Capabilities](../../../ai-capabilities/creating-custom-capabilities.md).
 :::
...
## Chart and graph responses

When your AI Chat Receptionist has access to the data behind a question — for example, through a connected Custom Capability — it can include a chart or graph image with its answer instead of describing the numbers in text. Line and bar charts are supported.

Charts appear as an image within the conversation, the same as any other response, across Web Chat, Email, SMS, WhatsApp, Facebook, and Instagram.

## How do I preview my AI Chat Receptionist in action? {#test-the-ai-chat-receptionists-responses}
...
### How do I monitor and improve the AI Chat Receptionist? {#monitor-and-improve-the-ai-chat-receptionist}

Regularly review the `Conversations` and `Anonymous Visitors` tabs in `Conversations` to make sure your Chat Receptionist is interacting with visitors as you expected.
...
<details>
<summary>Can the AI Chat Receptionist respond in Spanish or other languages?</summary>

Yes. The AI Chat Receptionist detects and responds in over 40 languages, including English, French, and Spanish. It replies in the same language the visitor uses.

</details>

<details>
<summary>How do I make the AI Chat Receptionist respond to Facebook messages?</summary>

Go to <AISparkleIcon /> `AI` > `AI Workforce` and click `Configure` on the Chat receptionist. In **Triggers**, assign the **Facebook** trigger so the AI automatically responds to inbound Facebook messages. You can assign **Email**, **SMS**, **Instagram**, and **WhatsApp** triggers the same way.

</details>

<details>
<summary>Where do I see the inquiries and leads from the AI Chat Receptionist?</summary>

Review the `Conversations` and `Anonymous Visitors` tabs in `Conversations`. Conversations go to `Anonymous Visitors` when the AI couldn't identify the visitor's contact information. Leads captured by the AI are saved directly to your CRM.
