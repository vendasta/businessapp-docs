...
Every Vibe project gets a default published URL automatically. On the Standard plan, you can also publish to a domain you own — including country and regional domains like `.co.uk`, `.com.au`, and `.org.uk`. TLS is provisioned automatically for both your default URL and any custom domain, so you don't need to purchase or manage certificates.
...
Custom domains require the Standard plan. If you're on a different plan, upgrade first through the self-checkout flow in Business App. See [Credits](../credits.md) for information on plan upgrades.
