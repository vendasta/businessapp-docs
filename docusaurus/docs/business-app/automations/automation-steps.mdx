...
Long automations get hard to scan. The Automation Builder has a set of canvas tools for this — step search, step references, and the minimap. See [Navigating the canvas](./navigating-the-canvas.mdx).
...
| Start a campaign for the custom object | Enrolls the contacts associated with a custom object record in a specified email campaign (up to 50). A contact associated with multiple records in the same automation run is enrolled once. If the record has no associated contact, the step is skipped and the reason appears in the automation activity feed |
...
| Step | Description |
|---|---|
| Notify a user | Sends an in-app and email notification to a team member. Select a specific user, or insert a dynamic user ID from a previous step so the notification routes to the right person automatically |
...
| Make outbound call with an employee | Places an outbound AI phone call to a contact using a selected AI Employee |
...
- [Review request automation](./review-requests) - Collect customer reviews automatically
- [Dynamic content](./dynamic-content-in-automations) - Personalize steps with dynamic contact and company data
