...
title: Steps
sidebar_position: 3
description: Browse all available workflow steps and learn how logic steps like delays, If/Else branches, and End This Workflow work.
---

Workflow steps are what happen after a trigger fires. Each step defines a specific action your workflow performs: sending a message, updating a record, creating a task, and so on. You can add multiple steps to a workflow and they run from top to bottom.
...
**Logic steps (purple)** control the flow of the workflow: they don't perform an action themselves, but decide what happens next. Use them to introduce delays, branch based on conditions, jump to a different point in the workflow, or end the run early. See the [Step Reference](#step-reference) for the full list.

<div style={{ display: 'flex', gap: '24px', justifyContent: 'center', alignItems: 'flex-start' }}>
  <img src={require('./img/action-steps.png').default} alt="Action steps in the workflow builder showing Companies, Contacts, Users, and Businesses categories in blue" width="45%" />
  <img src={require('./img/logic-steps.png').default} alt="Logic steps in the workflow builder showing Delays, Conditions, and Workflow categories in purple" width="45%" />
</div>
...
The **Delay** step pauses the workflow for a fixed amount of time before moving to the next step. Use it when timing matters, for example, waiting a day before sending a follow-up, or giving a contact time to respond before escalating.
...
<div style={{ textAlign: 'center' }}>
  <img src={require('./img/automations-step-delay.png').default} alt="Delay step in the Workflow Builder showing a 3-day wait before starting an SMS campaign" width="60%" />
</div>

### Delay until an event happens

The **Delay until an event happens** step pauses the workflow and waits for a specific event to occur before continuing, rather than waiting a fixed amount of time. Use it when the next step depends on something happening, not just time passing.
...
:::note
If the contact becomes ineligible for the workflow while waiting (for example, the workflow is turned off), the run may be cancelled.
:::
...
<div style={{ textAlign: 'center' }}>
  <img src={require('./img/automations-if-else-branch.png').default} alt="If/else branch in the Workflow Builder showing a Yes and No path based on a contact tag condition" width="60%" />
</div>
...
The **End This Workflow** step immediately stops the current run. It's most useful inside an If/Else branch when one path should proceed and the other should stop entirely, without needing to restructure your existing steps.
...
:::tip Hack: Use End Workflow to simplify restructuring
When you need to add an If/Else branch above a series of existing steps, you don't have to move each downstream step into the new branch. Instead, place the If/Else at the top, put **End Workflow** in the branch that should stop, and leave your existing steps in the main flow below. This saves you from manually reorganizing everything.
...
| Send a marketing email template to a contact | Sends a marketing email from a template to a contact's email address |
| Send a marketing email template to a company | Sends a marketing email from a template to the primary address on file for a company |
| Send a marketing email to a contact | Sends a standalone email to a contact |
...
| Step | Description | Special Cases |
|---|---|---|
| Delay | Pauses the workflow for a set amount of time before continuing to the next step | The workflow run may be cancelled if the contact or company becomes ineligible for the workflow while waiting |
...
| End this workflow | Immediately stops the current workflow run |

## Related resources

- [Workflow triggers](./automation-triggers) - Events that start your workflow
- [Workflow conditions](./automation-conditions) - Filter when your workflow runs using AND/OR logic
- [Messaging steps](./use-cases/action-messaging) - Send SMS and email messages via Conversations
- [Campaign steps](./use-cases/action-campaigns) - Pause and start campaigns from a workflow
- [Review request workflows](./review-requests) - Collect customer reviews automatically
- [Dynamic content](./smart-values-in-automations) - Personalize steps with dynamic contact and company data
- [Workflow settings](./automation-settings) - Control how often it runs and how errors are handled
