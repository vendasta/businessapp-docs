...
| Triggered via Zapier | Starts the automation when the **Run Automation** action is used in the Zapier integration. See [Zapier triggers Business App](./zapier/zapier-triggers-business-app) for setup instructions. | Trigger an automation from Zapier to update contacts in the CRM and pass along data from another app. |
