...
- **Value**: Use [dynamic content](../dynamic-content-in-automations.mdx) to insert the actual data from Business App, for example the company's name or country pulled from the trigger.
...
If the webhook step fails (for example, due to a network error or an invalid URL), the automation run logs an error. You can review run history in the [Activity](../automation-activity.md) tab. On the Zapier side, failed webhook events also appear in Zap History.
</details>
