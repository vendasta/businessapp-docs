...
description: Use the Send a webhook step in a Business App workflow to push data out to Zapier when something happens in Business App.
tags: [zapier, workflows, integrations, webhook]
keywords: [business app zapier, send webhook, workflow webhook, zapier webhook trigger]
---
...
- A Business App workflow with a `Send a webhook` step
...
**Step 7:** Go to `Business App` > `Workflows` and open the workflow you want to use, or create a new one. Make sure it includes a `Send a webhook` step: that's what pushes data out to Zapier.
...
<div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
  <img src={require('./img/businessApp-send-a-webhook-step.png').default} alt="Send a webhook option under the Advanced section in the Business App workflow step picker" width="75%" />
</div>
...
- **Value**: Use [dynamic content](../smart-values-in-automations.mdx) to insert the actual data from Business App, for example the company's name or country pulled from the trigger.
...
**Step 12:** Save and turn on your workflow.

**Step 13:** Trigger your workflow in Business App. For example, create a test company so Zapier receives a sample payload.
...
<details>
<summary>Can one workflow send data to multiple Zaps?</summary>

Yes. You can add multiple Send a webhook steps to a single workflow, each pointing to a different Zapier webhook URL. Each step fires in sequence when the workflow runs.
</details>

<details>
<summary>What happens if the webhook request fails?</summary>

If the webhook step fails (for example, due to a network error or an invalid URL), the workflow run logs an error. You can review run history in the [Activity](../automation-activity.md) tab. On the Zapier side, failed webhook events also appear in Zap History.
</details>
