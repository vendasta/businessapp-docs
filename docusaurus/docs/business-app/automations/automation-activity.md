...
sidebar_position: 5
description: View and troubleshoot past workflow runs using the Activity and History logs.
tags: [workflow, troubleshooting]
keywords: [workflow history, workflow logs, troubleshoot workflows, workflow success rate]
---

Use Workflow Activity to see what ran, when it ran, and whether it succeeded. This helps you verify outcomes and troubleshoot issues.
...
![Workflow activity tab showing recent runs](./img/automations-activity%20tab.png)

## Understanding the Activity log

The Activity tab displays a log of every time your workflow runs. Each row represents a step in the workflow and shows:

- **Contact** - The person or company record that triggered this workflow run
...
- **Activity** - The outcome of that step. This tells you what actually happened and whether it succeeded. Common activity statuses include:
  - **Workflow completed** - The run finished successfully
  - **Running** - The workflow is in progress and steps update in real time
  - **Entered workflow** - The workflow was triggered and started
...
1. **Is the workflow running?** In the workflow list, the state must be Running, not Stopped or Paused. Turn it on if needed.
...
5. **Was there an error?** Open the Activity tab for this workflow. Use the Entity search or Filters to narrow to the right time or contact if needed, then look in the Activity column for steps that errored or were skipped around the time the workflow should have run.

### "Workflow ran but step failed"

1. **Check the Activity tab** for the workflow. Use the Entity search or Filters to find the run if needed. Find the run that failed and look at each step’s Activity.
...
A workflow may stay in Running for a while when:
...
- **Reduce noise:** Add Conditions so the workflow runs only when needed.
- **Avoid duplicates:** Use the Entry settings (Only once / Multiple / One at a time) appropriately.

## Pro tips

- Test first: Trigger the workflow with a test record before enabling broadly
...
<details>
<summary>Why is my workflow stuck in "Running"?</summary>

A "Running" status usually means the workflow is waiting for something—for example, a delay step, a "Wait For" condition, or an external service. See [Workflow is stuck](#workflow-is-stuck) above for causes and what to check.
...
When asking for help with a workflow, have ready: the **workflow name**, **when** the issue happened (date and time), and **what you see in the Activity tab** (a screenshot of the run or failed step helps). If the problem involves a specific contact or record, note the state of that record at the time the workflow ran, not just how it looks now.
