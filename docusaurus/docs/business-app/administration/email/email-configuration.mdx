...
Configure your email settings via `Administration` → `Email Configuration`.
...
## Set a display address for emails

Anti-spam laws such as CAN-SPAM and GDPR require marketing and review request emails to include a physical mailing address in their footer. By default, this address comes from your [Business Profile](../business_profile.mdx).

If you'd rather show a different address, for example a mailing address instead of your service area, turn on `Use physical address override` under `Administration` → `Email Configuration` → `Display Address` and enter the address to show:

- `Address` and `Additional Address Line`
- `City`
- `Country` (required)
- `Postal / Zip Code`
...
![Display Address section with Use physical address override turned on](img/physical-address-override.png)

When this override is turned on, it takes priority over your Business Profile address in every email that shows a display address, including review request emails.
...
You must be signed in directly to Business App (not through a link that opens it for you). Then go to `Administration` → `Email Configuration` and update the email address and sender name there.
</details>
...
Check `Administration` → `Email Configuration` for verification status. Red exclamation points mean a record is not verified. Fix the DNS records (SPF, DMARC, DKIM) with your domain provider and wait for propagation.
</details>
