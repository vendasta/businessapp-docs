...
- Organize Layout — arrange fields into groups and set their display order for each object
...
Use Organize Layout to control how fields display on Contact, Company, and custom object records.

1. Go to `Administration` → `CRM Objects`.
2. Select an object (Contact, Company, or a custom object).
...
You can reset an object's layout back to its default configuration at any time.
