...
You can create up to 3 custom objects per account. See [Custom Objects](../../crm/custom-objects.md) for details. For more on extending records with custom fields, see [CRM Fields](./app_settings_crm_fields.md).
...
Creating a field decides what you can store. A field layout decides where it shows up. Group related fields into named sections, reorder them, and hide fields you don't use without losing their data.

To edit a layout, go to `Administration` > `CRM Objects`, choose an object, and click `Organize layout`.

See [Field layouts](./field-layouts.md) for the full guide.
