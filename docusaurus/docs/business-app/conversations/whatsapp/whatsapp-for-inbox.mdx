---
title: WhatsApp for Conversations in Business App
sidebar_label: "WhatsApp"
sidebar_position: 8
description: Learn how to connect WhatsApp Business account to Conversations for international communication capabilities in Business App.
---

## What is WhatsApp for Conversations?

You can connect your WhatsApp Business account to Conversations, giving you international communication capabilities. With this feature, you can send WhatsApp messages to new leads and existing customers, all while managing customer interactions in one centralized location.

![WhatsApp for Conversations AI Overview](../img/inbox/whatsapp_image_1.png)

## Why this matters

For businesses outside the U.S. and Canada, WhatsApp is a preferred communication channel for customers. With this integration, you can respond to leads on your customers' favorite platform, enhancing responsiveness and improving customer satisfaction.

## Availability

WhatsApp messaging channel is available in Conversations in Business App, for any accounts with either **Conversations AI Pro** or **AI-assisted Web Chat** products active on your account, internationally.

## What's included

- Direct integration with Conversations (send and receive WhatsApp messages directly from Conversations)
- WhatsApp message templates for outbound messaging (billed per message by Meta and subject to Meta's daily messaging limits)
- Unlimited incoming messages

## Setting up the WhatsApp integration {#setting-up-whatsapp-business-integration}

Follow these steps to connect your WhatsApp Business account to Conversations:

1. **Navigate to Conversations Settings:**
   - In Business App, go to `Administration` → `Conversations Settings`.
   - Find the card labeled `WhatsApp Business Messages`.
   - Click `Sign in with Facebook`.
     ![WhatsApp Sign in with Facebook](../img/inbox/whatsapp_image_2.png)

2. **Log in to Facebook:**
   - A modal will appear prompting you to sign in with your Facebook account.
     ![Facebook Login Modal](../img/inbox/whatsapp_image_3.png)
   - Allow Business App to connect by clicking `Continue as [Your Name]`.
     ![Continue as your name](../img/inbox/whatsapp_image_4.png)

3. **Connect your business account:**
   - Next, you'll be asked to connect your account. Click `Get Started`.
     ![Connect to your account](../img/inbox/whatsapp_image_5.png)

4. **Get your SMS/Phone number from Conversations:**
   - In Business App, go to `Settings` → `Conversations Settings` → `Phone & SMS` → `Configure`.
   - Take note of your number, as you'll need it to set up WhatsApp.
     ![Add phone number](../img/inbox/whatsapp_image_7.png)

:::note
You can use the assigned Conversations number if you're in a country where SMS is supported and your account has been assigned a number (USA/Canada).
:::

5. **Temporarily enable call forwarding:**
   - Go to `Conversations Settings` → `Your SMS/Phone Number` → `Settings`.
   - Enable **Forward the call** temporarily to receive the verification call.

6. **Verify ownership via phone call:**
   - Return to the WhatsApp setup screen.
   - Enter your Conversations number and select `Phone Call` as the verification method.
   - You will receive a verification call with a code.
     ![Verify phone number](../img/inbox/whatsapp_image_8.png)

7. **Restore call forwarding settings:**
   - After verification, disable call forwarding or return it to your original settings.

8. **Complete the setup:**
   - Confirm the app's ability to connect to WhatsApp on the final confirmation screen.
     ![Confirm connection](../img/inbox/whatsapp_image_9.png)
   - Wait for verification to complete.
     ![Verification in progress](../img/inbox/whatsapp_image_10.png)
   - Once successful, the Conversations AI settings will show a green 'connected' badge.
     ![Connected status](../img/inbox/whatsapp_image_11.png)
     ![Connected badge](../img/inbox/whatsapp_image_12.png)

## Sending messages through WhatsApp

Your options for sending a WhatsApp message are different based on your conversation history with your customer:

- If you are sending a WhatsApp message to someone who **has contacted you first**, you are free to write any message that meets WhatsApp guidelines.
- If you are sending a message to someone who **has not contacted you first**, you must reply using a WhatsApp template.

If your account is new and does not yet have any templates, the system will automatically submit a template for approval on your behalf. You can also create and submit additional templates anytime through the [Meta Business Help Center](https://business.facebook.com/latest/whatsapp_manager/message_templates).

:::info
Only approved templated messages can be sent as outbound cold messages. Once a customer initiates contact, you can reply with custom messages within a 24-hour window.
:::

**Key Notes:**

- Outbound messages are billed by Meta, not Business App.
  ![Template management](../img/inbox/whatsapp_image_13.png)

## Enhancing your presence on WhatsApp

Some advanced features and upgrades are controlled directly by Meta (WhatsApp's parent company). We provide this information to help you understand these options and how to access them.

### Verified WhatsApp Business Accounts

Verified businesses get benefits such as:
- Increased messaging limits (from 250 to 1,000 daily business-initiated conversations)
- Business name displayed in customer chats
- Higher trust and visibility within WhatsApp

To apply for verification, visit Meta's [WhatsApp Manager](https://business.facebook.com/latest/whatsapp_manager).
![WhatsApp verification](../img/inbox/whatsapp_image_14.png)

## Getting WhatsApp replies

- Customer replies to your WhatsApp Business account automatically appear in the same Conversations thread
- Business App retains all WhatsApp history alongside other channels like email, SMS, and web chat
- Any team member can view and respond to WhatsApp conversations from the shared workspace

## Messaging limits

- **Unverified Businesses:** Limited to 250 business-initiated conversations per day.
- **Verified Businesses:** Can initiate up to 1,000 conversations daily and access additional features.
- High-quality messaging practices can lead to increased limits over time.
  ![Messaging limits](../img/inbox/whatsapp_image_15.png)

## Billing information

Access to WhatsApp through Conversations AI is included with Conversations AI Pro and AI-Assisted Web Chat and allows:

- Unlimited incoming messages.
- Unlimited replies to a customer within the 24-hour customer service window, at no additional charge.

Business App does not charge for your outbound WhatsApp messages. Meta bills outbound messaging directly.

### How outbound messaging is charged

Meta charges for each business-initiated template message you send (for example, marketing, utility, and authentication messages). The cost depends on the message category and the recipient's country. Messages you send inside an open 24-hour customer service window—that is, after a customer messages you first—are not charged.

The **250** and **1,000** figures are daily messaging limits—the maximum number of business-initiated conversations you can start per day (see [Messaging limits](#messaging-limits) above). They are not an allowance of free messages. Template messages are billed by Meta from the very first message you send.

:::warning
Outbound message charges are billed directly by Meta and require a valid payment method in your Meta Business account.
:::

[Learn more and see the rate card.](https://developers.facebook.com/docs/whatsapp/pricing#rate-cards)

![Billing information](../img/inbox/whatsapp_image_16.png)

## Related documentation

[FAQs | WhatsApp Business](https://business.whatsapp.com/resources/faq)

For more details, visit the [Meta Business Help Center](https://business.facebook.com/latest/whatsapp_manager/).

## FAQs

<details>
<summary><strong>Can I connect an existing WhatsApp business account to Conversations AI, and still use the mobile app?</strong></summary>

Yes. You can use **WhatsApp Coexistence** to link your existing WhatsApp Business App (on your phone) to Conversations while keeping the app on your phone. See [WhatsApp Coexistence](./whatsapp-coexistence) for setup steps. If you prefer not to use coexistence, you can register a separate WhatsApp Business account for use with Conversations.
</details>

<details>
<summary><strong>Why can't I write an outgoing message?</strong></summary>

Only approved templated messages can be sent as outbound cold messages. Once a customer initiates contact, you can reply with custom messages within a 24-hour window.
</details>

<details>
<summary><strong>Why did my message fail to send?</strong></summary>

Your template may not be approved. You can check the status of approved templates in [WhatsApp Manager](https://business.facebook.com/latest/whatsapp_manager/message_templates).
</details>

<details>
<summary><strong>What will WhatsApp messaging cost me?</strong></summary>

Replies you send to a customer within the 24-hour customer service window (after they message you first) are free. Business-initiated template messages are billed by Meta from the first message you send, based on the message category and the recipient's country. The 250 and 1,000 figures are daily messaging limits, not free monthly messages. A valid payment method must be added to your Meta account. See Meta's [rate card](https://developers.facebook.com/docs/whatsapp/pricing#rate-cards) for current pricing.
</details>
