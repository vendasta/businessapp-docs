...
When Reputation AI records an `NPS feedback` or `Review matched` activity for a contact, it appears in that contact's Conversations timeline.

If a contact updates their NPS response, Conversations keeps a single `NPS feedback` activity and updates it with the latest score and comment.

These activities appear alongside your messages, so you can track customer sentiment and review attribution in one place.
...
For more details, see [AI Web Chat Overview](./web-chat/index.mdx).
