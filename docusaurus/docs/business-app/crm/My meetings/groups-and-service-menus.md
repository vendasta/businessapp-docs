...
- Redirect doesn't apply to meetings you book yourself from a CRM contact, because those bookings don't show a public confirmation page.
...
### AI booking limitations

:::warning
AI Chat and AI Voice Receptionists book on the customer's behalf, so some options from your booking page don't apply to AI bookings:

- **Client selection**: The customer isn't offered a choice of team member. For event types set to Client selection, the receptionist assigns an available team member automatically.
- **Custom questions**: The receptionist doesn't ask your custom questions, so those answers aren't captured on the booking. It still collects the default details it needs to confirm the booking, such as the customer's name and phone number or email.
- **Notes**: The receptionist doesn't collect the `Additional comments` that customers can add on the booking page.

If you need extra details from customers who book through a receptionist, ask for them in the receptionist's instructions or follow up after the booking.
:::

:::note
Receptionists can only offer event types that have at least one host with both a calendar and a meeting app connected. Event types with no connected host are left out of the receptionist's options without a warning, and a Group is left out entirely if none of its event types qualify. If a service isn't being offered, check the hosts' calendar and meeting app connections first.
:::
...
**Chat booking requirements:** Every event type you want the Chat Receptionist to offer needs at least one host with a connected calendar and meeting app. Event types without a connected host don't appear in the receptionist's options. See [AI booking limitations](#ai-booking-limitations).
...
**For sales teams:**
- Group related meeting types together, for example Discovery Call, Product Demo, and Pricing Review in one "Sales Meetings" group.
...
- Existing event types can be added to Groups and Service Menus immediately. There's no need to recreate them.
