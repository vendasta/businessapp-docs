...
Use AI to quickly create short and long business descriptions from the `Descriptions` tab in `Business Profile`. This feature works when Local SEO and/or Listing Sync Pro is active.

1. Go to `Business Profile` and select the `Descriptions` tab.

   ![Business Profile descriptions tab](./img/businessprofile-description.png)
2. Click `Suggest Description`.

   ![Suggest Description button in descriptions tab](./img/businessprofile-shortdescription.png)
3. Use the dropdown to explore options. Select `Create from current content` to build from an existing description.

   ![Description options menu including create from current content](./img/description-create-from-current-content.png)
4. Select `Improve Text` to fix spelling and grammar, shorten the text, or adjust tone.

   ![Improve Text options for editing description](./img/description-improve-text.png)
5. Choose a tone such as friendly, professional, or witty.

   ![Tone options like friendly, professional, witty](./img/description-change-tone.png)
...
![Long Description field](./img/businessprofile-long-description.png)
...
2. Enter your hours, including ranges that extend past midnight (for example, `11:00 am - 3:00 am`).
   ...
   ![Configuring hours that extend past midnight](./img/businessprofile-hours-of-operation.png)
...
![Business Profile Hours tab showing Special Hours](./img/businessprofile-special-hours.png)

![Business Profile More tab showing category-specific attributes](./img/businessprofile-attributes.png)
...
2. Add category-specific hours such as delivery, drive-through, takeout, or pick-up.
   ...
   ![More Hours options within Hours tab](./img/businessprofile-more-hours.png)
...
1. In the `Business Profile` section of account creation, check `Service Area Business (hide physical address)` (below `Location Map`). It is turned off by default.
   ...
   ![Service Area Business option during account creation](./img/businessprofile-hide-address.png)
...
If a business has temporarily or permanently closed, you can update this status in Business Profile so the change syncs across all connected directories.

1. Open `Accounts` and select the business.
2. Click `View all data`.
3. Under `Primary info`, go to `Operational Information`.
4. Select the appropriate status from the **Business open/closed** dropdown.
5. Click `Save`.

Once saved, the closed status syncs to all listing sources managed by the active listing sync product.

**Temporarily closed**: Use this when the business will reopen. When the business reopens, return to the same setting and change the status back to open.

**Permanently closed**: Use this when the business will not reopen. Note that some directories handle permanently closed listings differently — for example, Apple Maps will keep permanently closed listings visible but removes them from category searches and places them below open businesses. Apple will eventually remove them entirely.

### How logo and primary photo sync to Google Business Profile

Images tagged in the Business Profile Media section sync to Google Business Profile as follows:

- **Logo** → syncs as the **Profile photo** on Google Business Profile
- **Primary photo** → syncs as the **Cover photo** on Google Business Profile

If your logo or cover photo is not displaying on Google, the most common cause is that the image does not meet Google's dimension or format requirements. Images that fall outside the requirements are not displayed on Google Maps or the Google listing. See the image requirements below for the correct specifications.

Not all directories in the listing sync network accept logo or gallery photos — photo support varies by source.
...
![Google Business Profile image requirements overview](./img/gbp-image-requirements.png)
...
<details>
<summary>How do I mark a business as temporarily or permanently closed?</summary>

Go to `Accounts`, open the business, click `View all data`, and under `Primary info > Operational Information`, select the status from the **Business open/closed** dropdown. The change syncs to all connected listing sources.

</details>

<details>
<summary>How do I reopen a business that was marked as temporarily closed?</summary>

Return to `Accounts` > `View all data` > `Primary info` > `Operational Information` and change the **Business open/closed** status back to open. The updated status will sync across connected directories.

</details>

<details>
<summary>How does my logo appear on Google Business Profile?</summary>

Your logo syncs as the **Profile photo** on Google Business Profile. Your primary photo syncs as the **Cover photo**. If either image is not displaying, check that it meets Google's image size and format requirements — images outside the required dimensions are not displayed on Google Maps or the listing.

</details>

<details>
<summary>Why isn't my logo or primary photo showing on Google?</summary>

The most common reason is that the image does not meet Google's dimension or format requirements. Verify the image is in JPG or PNG format, between 10 KB and 5 MB, and meets the minimum dimension requirements for logo or cover photo. Also note that not every directory in the listing sync network accepts logo or gallery photos — this is source-specific.

</details>

<details>
<summary>Why doesn't my primary category in Local SEO match Google Business Profile?</summary>

Local SEO does not sync the primary category to Google. A mismatch usually means the Google value was set directly in Google Business Profile or by another provider. Changing a primary category can risk a Google suspension, so treat that field carefully.
