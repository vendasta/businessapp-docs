      ...
      'classic',
      {
        ...
        docs: {
          ...
          async sidebarItemsGenerator(args) {
            // Let Docusaurus build the items first
            const items = await args.defaultSidebarItemsGenerator(args);

            // Only adjust autogenerated slices
            if (args.item.type !== 'autogenerated') return items;

            const dirName = args.item.dirName; // e.g. "conversations"
            const docsById = Object.fromEntries(args.docs.map(d => [d.id, d]));

            const isRootIndexLike = (docId: string) => {
              const doc = docsById[docId];
              if (!doc) return false;
              // Only consider docs that live at the *root* of this slice
              if (doc.sourceDirName !== dirName) return false;

              const base = path.basename(doc.source).toLowerCase(); // e.g. "index.md"
              return (
                base === 'index.md' ||
                base === 'index.mdx' ||
                base === 'readme.md' ||
            ...
            // Filter out root index-like files from the items
            return items.filter(item => {
              if (item.type === 'doc') {
                return !isRootIndexLike(item.id);
              }
              return true;
            });
          },
